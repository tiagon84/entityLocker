...
public class EntityLockerTest extends Assert {
    ...
    public void testTimeouts() throws InterruptedException {
        ...
        locker.tryLock(1, 50, () -> thread2.set(true));
        t1.join();
    ...
    /**
     * Implement protection from deadlocks (but not taking into account possible locks outside
     * EntityLocker)
     */
    @Test
        ...
        Thread t1 = new Thread(() -> {
            try {
                for (int i = 0; i < ITERATIONS; i++) {
                    entityLocker.lock(1, () -> {
                        try {
                            entityLocker.lock(2, counter12::incrementAndGet);
        ...
        Thread t2 = new Thread(() -> {
            for (int i = 0; i < ITERATIONS; i++) {
                try {
                    entityLocker.lock(2, () -> {
                        try {
                            entityLocker.lock(1, () -> innerCodeCalled.set(true));
                        } catch (InterruptedException e) {
